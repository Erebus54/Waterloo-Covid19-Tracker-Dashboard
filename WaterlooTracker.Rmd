...
fig <- fig %>% layout(xaxis = list(title = ""),
                      yaxis = list (title = "Cumulative Cases (Log)"))
fig
```
...
plot_ly(data = outcomes_changes, x = ~Case_Reported_Date, y = ~new_deaths, 
        type = 'scatter',
        mode = "lines+markers", 
        name = "New Deaths", 
        marker = list(color = "#FF5959", size = 1), 
        line = list(color = "#FF5959", width = 1)) %>% 
  
  add_trace(y = ~new_resolved, 
            name = "New Resolved", 
            marker = list(color = "#6EC5E9", size = 1), 
...
daily_ages <- TargetPHU %>% 
  dplyr::filter(!is.na(Case_Reported_Date)) %>% 
